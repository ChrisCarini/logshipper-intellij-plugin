...
public class LogshipperConfigurableUI implements ConfigurableUi<SettingsManager.Settings> {
    ...
    private final JBTextField reconnectDelayField = new JBTextField();
    ...
    private void buildMainPanel() {
        ...
        final JButton copyExampleConfigToClipboardButton =
                new JButton(Messages.message("button.copy.to.clipboard"));
        ...
        mainPanel.add(FormBuilder.createFormBuilder()
                ...
                .addLabeledComponent(Messages.message("title.logstash.settings.hostname"),
                        hostnameField)
                .addLabeledComponent(Messages.message("title.logstash.settings.port"), portField)
                .addLabeledComponent(
                        Messages.message("title.logstash.settings.reconnect.delay.in.ms"),
                        reconnectDelayField)
                .addLabeledComponent(
                        Messages.message("title.logstash.settings.include.log.location.info"),
                ...
                .addLabeledComponent(
                        Messages.message("title.helpful.debug.settings.generate.sample.log.messages"),
                ...
                .addComponent(
                        new TitledSeparator(Messages.message("title.sample.logstash.configuration")))
                .addComponent(new JBLabel(
                        Messages.message("title.sample.logstash.configuration.description")))
    ...
    public void reset(@NotNull final SettingsManager.Settings settings) {
        ...
        final String reconnectionDelay;
        if (settings.reconnectDelay == null || settings.reconnectDelay.isEmpty()) {
            reconnectionDelay = Integer.toString(LayoutSocketAppender.DEFAULT_RECONNECT_DELAY);
            settings.reconnectDelay = reconnectionDelay;
        } else {
            reconnectionDelay = settings.reconnectDelay;
        }

        hostnameField.setText(settings.hostname);
        portField.setText(settings.port);
        reconnectDelayField.setText(reconnectionDelay);
    ...
    public boolean isModified(@NotNull final SettingsManager.Settings settings) {
        ...
        final boolean sameReconnectDelay = reconnectDelayField.getText().equals(settings.reconnectDelay);
        ...
        return !sameHostname || !samePort || !sameReconnectDelay || !sameAddConsoleAppender
                || !sameGenerateSampleLogMessages;
    }
    ...
    public void apply(@NotNull final SettingsManager.Settings settings) {
        ...
        settings.reconnectDelay = reconnectDelayField.getText();
